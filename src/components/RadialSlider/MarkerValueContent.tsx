      ...
      <>
        {marks.map((mark: { value: number }, index: number) => {
          ...
          const getTransformValue = () => {
            return `rotate(89.5) translate(${twoDigitsPositionValue})`;
          };
          ...
          const getTextPositionValue = (type: string) => {
            if (mark?.value < centerValue) {
              ...
              return type === "x" ? twoDigitsPositionValue : "-150";
            }
          };
            ...
            <G key={index.toString()}>
              {(index % markerInnerValue === 0 || index === markIndex) && (
                <G
                  ...
                  }, ${radius + (lineHeight - thumbBorderWidth) - 2})  `}
                >
                  <SVGText
                    fontSize={16}
                    transform={`rotate(${index * maxCount + 87 + angle})  
                    ${getTransformValue()}`}
